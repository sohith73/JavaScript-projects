...
function updateProfile(data) {
    ...
    if (data.message !== "Not Found") {
        ...
        if(input.value === "sohith73"){
            date.innerText = `Joined 1st December 2021 `;
            repos.innerText = data?.private_repos;
        }
        else{
            date.innerText = `Joined ${dateSegment[2]} ${month[dateSegment[1] - 1]} ${dateSegment[0]}`;
        }

        profileBio.innerText = (data?.bio === null) ? "This Profile has no Bio bro!" : data?.bio;;
